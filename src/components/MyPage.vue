...
<script>
    ...
    export default {
        ...
        setup(){
            var asset_ldzc=ref('流动资产'+nanoid())
            var asset_ldzc_total=ref(12000)
            var asset_gdzc=ref('固定资产')
            var asset_gdzc_total=ref(40000)
            var asset_total=ref(0)
            var asset_ldzc_mx=reactive({xj:{},ch:{}})

            function sumzcze() {
                asset_ldzc_total.value=Math.ceil(Math.random()*1000000)
                asset_gdzc_total.value=Math.ceil(Math.random()*1000000)
                asset_ldzc_mx.xj={title:'现金',total:Math.ceil(Math.random()*1000000)}
                asset_ldzc_mx.ch={title:'存货',total: asset_ldzc_total.value-asset_ldzc_mx.xj.total}
